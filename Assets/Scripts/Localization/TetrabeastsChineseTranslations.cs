...
public static class TetrabeastsChineseTranslations
{
    ...
    static void AddTopLevelText(Dictionary<string, string> text)
    {
        ...
        Add(text, "{0} dies.", "{0}死亡。");
        Add(text, "{0} uses {1}.", "{0}使用{1}。");
        Add(text, "{0} casts {1}.", "{0}施放{1}。");
        Add(text, "{0} took {1}{2} damage{3}.", "{0}受到{1}{2}伤害{3}。");
        Add(text, "{0} restored {1} health for {2}.", "{0}为{2}恢复{1}点生命。");
        Add(text, "{0} dealt {1} damage to {2}.{3}", "{0}对{2}造成{1}点伤害。{3}");
        Add(text, " from {0}", "，来源：{0}");
        Add(text, "(shielded)", "（受护盾保护）");
        Add(text, "poison", "毒素");
        Add(text, "fire", "火焰");
        Add(text, "lightning", "闪电");
        Add(text, "contagion", "传染");
        Add(text, "starvation", "饥饿");
        Add(text, "burst", "爆发");
        Add(text, "floor effect", "地面效果");
        Add(text, "storm", "风暴");
        Add(text, "infection", "感染");
        Add(text, "low rations", "补给不足");
        Add(text, "death burst", "死亡爆发");
        Add(text, "spikes", "尖刺");
        Add(text, "Enemy Archer", "敌方弓箭手");
        Add(text, "rear ambush", "后方伏击");
        Add(text, "Castle", "城堡");
        Add(text, "Skybreaker Edict", "破天敕令");
        Add(text, "Heaven's Judgement", "天罚审判");
        Add(text, "Stormcaller's Verdict", "唤雷者裁决");
        Add(text, "Hex of the Warped Ground", "扭曲大地咒");
        Add(text, "Aegis of the Unbroken Crown", "不破王冠之盾");
        Add(text, "Temporal Distortion", "时间扭曲");
        Add(text, "Ward of the Arcane Pylons", "奥术塔柱守护");
        Add(text, "Rune of Ruin", "毁灭符文");
        Add(text, "Summon Earthen Rampart", "召唤土垒");
        Add(text, "Raise Iron Thorns", "升起铁刺");
        Add(text, "Sow Venomous Miasma", "播撒毒雾");
        Add(text, "Kindle Infernal Sigils", "点燃地狱印记");
        Add(text, "Call Stormbound Sigils", "召唤风暴印记");
